# -*- coding: utf-8 -*-
#
# Lecture des fichiers "tÃ©lÃ©com"
# Sylvain Lobry 2015
# JM Nicolas 2015#
#
...
#
# Aout 2018 : possibilitÃ© de lire un fichier url
#
##########################################################################
#
...
##########################################################################
#
#  Sauvegarde au fomat TÃ©lÃ©com : mat2imz
#
##########################################################################
#
...
#########################################################################
u"""
lecture, Ã©criture et affichage d'images Ã  forte dynamique, rÃ©eles ou complexes (radar)
"""

MVALABVERSION=u"V2.1  Version du 5 fÃ©vrier 2019"
...
def imz2mat(imgName,ncan=0):
    ...
    argument 2 (facultatif) : si multicanal, renvoie uniquement le canal indiquÃ© (ATTENTION : numÃ©rotation Ã  prtir de 0)
    """
...
#
    ...
    print(u'imz2mat : version janvier 2018.  Fichier Ã  ouvrir : %s'%imgName)
    ...
    try :
        ...
        print(legx)
        print(u'Echec Ã  l\'appel de imz2mat')
        return 0,0,0,0
...
##############################################################################
# Mai 2018 : donnÃ©es .cos du DLR
#
    if imgName.endswith('.cos') :
        return( cos2mat(imgName ))  # voir Ã  la fin de ce fichier, avant les outils de visu
...
#
    ...
    try :
        ...
        legx=imgName+': est un fichier non ouvrable'
        print(legx)
        print(u'Echec Ã  l\'appel de dimdat')
        return 0,0,0,0
...
#
    ...
    try :
        ...
        legx=imgName+': est un fichier non ouvrable'
        print(legx)
        print(u'Echec Ã  l\'appel de cos2mat')
        return 0,0,0,0
...
#    imgcxs = npy.empty([nlig-1, ncol], dtype=npy.complex64())
#
## on elimine les deux premiÃ¨res lignes : cf la doc !!
    fin.seek(0)
...
##########################################################
# AOUT 2018
# Lecture d'une partie des images TÃ©lÃ©com Paristech vua url
#
#########################################################
...
def urlimz2mat(nomurl, ncanselect) :
    ...
    # attention : tagcmplx est Ã©gal Ã  1 par dÃ©faut
    #   est Ã©gal Ã  2 si complexe
    #   est Ã©gal Ã  0 si couleur rvb
    ...
    if versionpython[0] == '2' :
        try :
            ...
            print(u'Erreur Ã  la lecture de %s :'%namedim,  e.reason)
            return 0,0,0,0,0
    ...
    nkan=1
    nn=len(datadim)
    if nn > 2 :   # le .dim a une premiÃ¨re ligne "complÃ¨te"
        nplan=int(datadim[2])
...
#####################################
    if versionpython[0] == '2' :
        try :
            ...
            print(u'Erreur Ã  la lecture de %s :'%nomurl,  e.reason)
            return 0,0,0,0,0
        dataficima=urllib.urlopen(nomurl )
...
#        print(u'Images rvb non traitÃ©es pour le moment')
#        return 0,0,0,0,0
...
# ????         return img, largeur, hauteur, 1, 1
    ...
    if nkan>1 and ncanselect>0 :
        print(u'SÃ©lection du canal %d/%d'%(ncanselect,nkan))
        if tagcmplx==1 :
...
#
# il faut Ã©ventuellement recentrer les .cxb
...
def dimimabase(imgName):
    """
    Renvoie les 4 paramÃ¨tres d'une image
    ParamÃ¨tre en entrÃ©e : le nom de l'image avec son extension (soit extensions telecom, soit .cos --Terrasar-X--, soit .dat --Ramses--)
    Sortie : une liste avec les 4 PARAMÃˆTRES  nombre de colonnes,  nombre de lignes, nombre de canaux, nombre de plans
    """
...
# d'oÃ¹ ipart, mais qui sert aussi Ã  neutraliser les affichages
#
# Sept 2018 : zparam peut aussi Ãªtre un tableau [vmin,vmax]
#
#
# therest :
...
#        print("visusar sans second parametre'
    ...
    if ZZ==3:  # on traite Ã  part les couleurs
        ...
        if tagspe > 0 :
            plt.figure()
            if ipart==1 :
                print(u'Figure rÃ©duite pour l\'image')
...
#  par defaut visusarbase affiche avec un seuil de vmoy + kparam * vsig et avec kparam = 3
    ...
    if type(paramseuil) == list :
        ...
        if kparamhisto==0 :
            kparamhisto=kparam
        print(u'seuil min (%.1f) et seuilmax (%.1f) passÃ©s en argument. kparam = %.1f'%(seuilmin, seuilmax, kparam))
        malegende=u'Image affichÃ©e entre %.1f et %.1f  (vmoy + %.1f vsig)'%(seuilmin,seuilmax,kparam)
    else:
    # ancien cas
        zparam = paramseuil
...
#            plt.suptitle(malegende)
#        if ipart==2 :
#            plt.title(malegende)  # placement trÃ¨s subtil, avant l'Ã©ventuel histogramme
#
        plt.set_cmap('gray')
        if len(therest)>2 :
            plt.set_cmap('jet')
...
#                seuilmaxhisto = valmax   # octobre 2018 : source d'erreur pour l'opÃ©rateur
...
def visusar(tabima,zparam=-999):
    ...
    Arguments en entrÃ©e : 1 ou 2
        ...
        Si ce facteur est nÃ©gatif, seuillage <<valeur moyenne - fak * Ã©cart type ; valeur moyenne + fak * Ã©cart type >>

        argument 3 (facultatif) :
            si nul : pas de plt.figure, ni de plt.show dans la procÃ©dure
    ...
    Utilisez visusarW (mÃªme syntaxe) pour n'avoir aucun affichage : on rÃ©cupÃ¨re le tableau qui aurait du Ãªtre affichÃ©
    """
# et si c'Ã©tait un nom d'image ???
...
def visusarspectre(image,*therest):
    u'''
    Une image est passÃ©e en premier argument

    l'image et son spectre sont tracÃ©es

    le second argument (facultatif) est une lÃ©gende
    '''
...
def visuflicker(ima1, ima2) :
    """
    Deux images en paramÃ¨tre
    cliquer sur la souris pour flicker l'image,
    entrer un caractÃ¨re au clavier pour sortir
...
# on canibalise visupile
    ...
    while istop==0 :
        ...
        if kut==1 :
            visusarZ(ima2)
        plt.gcf().canvas.set_window_title(u'Image %d/%d \n Souris pour dÃ©filer, clavier pour arreter'%(kut+1, nkan0))
    #    plt.draw()
        figpile.canvas.draw()
